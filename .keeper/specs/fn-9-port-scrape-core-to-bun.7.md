...
## Done summary
Added the AGENTUSAGE_NOW clock-injection seam to agentusage/scrape_cli.py (threaded into the parser only when set, so the absent path stays the exact one-arg call the pre-existing single-arg parser mocks rely on; a malformed value maps to scrape_failed, mirroring the Bun CLI). Built the parity proof against the identical stdout contract: tests/test_bun_parity.py compares the Bun parse-bridge vs in-process Python parse over all 14 corpus cases plus 9 inline parser fixtures (deep-equality on success, error_type-name equality on raises); tests/test_dual_run_cli.py drives BOTH CLIs end-to-end through the fake TUI over every corpus scenario plus the auth-probe fork (loggedIn true/false/garbage), a mount-delay ready-wait race, and a stubborn-child reaping case, asserting deep-equality, exact resets_at, one stdout line with trailing newline, matching exit codes, and stdout==expected.json. tests/conftest.py carries the bun/tmux skip discipline with AGENTUSAGE_REQUIRE_PARITY=1 promoting every reasoned skip to a failure. Extended tests/fake_tui.py additively (garbage auth_status_stdout override; a fork_child stubborn grandchild reaped via the controlling-terminal SIGHUP, the cross-platform reap since macOS getpgid ESRCHes on the zombie fake; a configurable session cap that outlives the tmux driver sentinel budget). Regenerated the corpus (zero drift) and refreshed the corpus_schema.md clock note; the four pre-existing test files are untouched. Acceptance on this box: uv run pytest = 167 passed / 1 deselected (parity active); AGENTUSAGE_REQUIRE_PARITY=1 on the two parity modules = 45 passed / 0 skips; tmux-absent yields reasoned skips (promoted to failures under REQUIRE); uv run pytest -m live = 1 passed (real claude scraped through both CLIs); ruff-format + ty + biome + tsc clean. Source commit fd7b0f6.
## Evidence
