...
/** Titlecase a single word, mirroring Python `str.title()` for one token. */
...
function parseBlock(
  ...
  // Validate the IANA zone up front, mirroring Python's explicit ZoneInfo()
  // guard: an unknown zone surfaces as a strict parse error, distinct from the
  // resolver's own calendar-overflow throw.
  try {
