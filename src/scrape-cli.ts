 ...
 * A 1:1 port of `agentusage/scrape_cli.py`: identical argv surface and the exact
 * discriminated JSON contract keeper's usage-scrape-runner parses.
 *
 *     bun src/scrape-cli.ts --target <claude|codex> --profile <name> \
 ...
 * (Python `.astimezone()`), so it needs a real IANA zone; codex keeps `now`'s
 * own fixed offset (Python resolves via `replace`, never reprojecting).
 */
function buildNow(
  target: Target,
 ...
 * routes through the agentwrap-profile shim; the default account is native
...
/** Strict base-10 integer, mirroring argparse's `type=int` (rejects `4.0`). */
