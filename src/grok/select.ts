  ...
  if (options.account) {
    ...
    for (const account of state.accounts) {
      try {
        ...
        rejected.push({ accountKey: account.accountKey, reason: error instanceof GrokError ? error.code : "ineligible" });
      }
    }
    if (candidates.length === 0) {
      throw new GrokError("no_eligible_account", "No Grok account is currently eligible for selection", { candidates: rejected });
    }
...
function candidateFor(account: StoredAccount, reservations: Set<string>, allowUnknown: boolean, now: number, exact: boolean): Candidate {
  ...
  if (!account.enabled) throw new GrokError("account_disabled", `Grok account ${account.accountKey} is disabled`, detail);
  if (account.observation.error?.code === "auth_unavailable") {
    throw new GrokError("auth_unavailable", `Grok account ${account.accountKey} was rejected by the billing service`, detail);
  }
  if (!account.credentials.accessToken || !account.credentials.refreshToken || account.credentials.expiresAtMs <= now) {
    throw new GrokError("auth_unavailable", `Grok account ${account.accountKey} does not have a currently valid session`, detail);
  }
  if (reservations.has(account.accountKey)) {
    throw new GrokError("account_reserved", `Grok account ${account.accountKey} is temporarily reserved`, detail);
  }
  ...
  if (included === null) {
    ...
    throw new GrokError("usage_unknown", `Grok account ${account.accountKey} has incomplete included-capacity data`, detail);
  }
  ...
  if (paygCapIncomplete) {
    ...
    throw new GrokError("usage_unknown", `Grok account ${account.accountKey} has incomplete PAYG cap data`, detail);
  }
  ...
  if (capacityUnknown) {
    ...
    throw new GrokError("usage_unknown", `Grok account ${account.accountKey} has incomplete capacity data`, detail);
  }
  throw new GrokError("account_exhausted", `Grok account ${account.accountKey} has no remaining included or paid capacity`, detail);
}
...
function validateReservationSeconds(value: number): void {
  if (!Number.isSafeInteger(value) || value < 1 || value > 300) {
    throw new GrokError("invalid_argument", "--reserve-seconds must be an integer from 1 through 300");
  }
}
