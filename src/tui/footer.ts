...
export function footerCopy(state: FleetFooterState): FooterCopy {
  ...
  const actionWidth = Math.max(1, innerWidth - (visibleMode ? visibleMode.length + 2 : 0));
...
export function createFleetFooter(
  ...
  const pressActionAt = (x: number): void => {
    for (const child of actionsBox.getChildren()) {
      ...
      currentActions.find((action) => `${id}-action-${action.id}` === child.id)?.onPress();
      return;
    }
  };
  ...
  return {
    ...
    update(state) {
      ...
      actionsBox.width = Math.max(1, innerWidth - (modeWidth ? modeWidth + 2 : 0));
        ...
        target.add(
          new core.TextRenderable(renderer, {
            content: new core.StyledText([
              ...
              ...(label ? [core.fg(palette.muted)(` ${label}`)] : []),
            ]),
          }),
        );
