/**
 * bun:test translation of tests/test_scrape_cli.py — the discriminated-arm
 * contract of run() plus the pure scrape-cli helpers, all in-process. run()'s
 * scrape / auth / emit collaborators are injected through the RunDeps seam, and
 * claudeAuthLoggedIn's subprocess through an AuthProbe, so NO subprocess spawns
 * and NO stdout pipe is read (which stays empty inside bun test — Bun#24690).
 * The emitted payload OBJECT is captured directly; arm shape + key-absence rules
 * assert against it. Titles keep the original pytest case names.
 *
 * Dead-at-translation (out of scope here — these are the subprocess-stdout /
 * argv-error contract cases, which land in the conformance-runner task, not this
 * pure-in-process file):
 *   - test_writes_no_state — statelessness is a real-process property; asserted
 *     end-to-end where the CLI actually spawns.
 *   - test_main_requires_target_and_profile — argv-error exit contract (exit 2 /
 *     stderr message) belongs to the end-to-end CLI runner.
 *   - the `_capture` "exactly one stdout line" assertion — a serialized-stdout
 *     property; its in-process analog (emit called exactly once) is asserted via
 *     the payload-count checks below.
 *   - test_default_claude_command_falls_back_for_non_string — the TS Target type
 *     makes `command` a string by construction; there is no non-string branch to
 *     fall back from.
 */
...
import { CodexStatusParseError, PANEL_SENTINEL } from "./parse-codex-status";
import {
  type AuthProbe,
  claudeAuthLoggedIn,
  hasPanelEvidence,
...
describe("scrape-cli — pure helpers", () => {
  ...
  test("test_passthrough_translation", () => {
    // Named Claude profiles route through the agentwrap-profile shim; the default
