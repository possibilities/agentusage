 ...
 *
 * A 1:1 port of the Python `scrape.py` pexpect+pyte driver onto a tmux driver
 * (ADR 0001): tmux owns the PTY, VT100 rendering, and child reaping in C; this
 * module shells out to a dedicated named server (`tmux -L agentusage-scrape`)
 * and reads the rendered screen with `capture-pane`. The sentinel state machine,
 * timing constants, keystroke shape, trust pre-marking, and cleanup all port
 * verbatim from the Python original; only the PTY/screen primitives change.
 */
...
// Snapshot-idle cadence (ADR 0001): a screen is "quiet" once N consecutive
// `capture-pane -pJ` snapshots taken this far apart are byte-identical. The
// pyte byte-silence model becomes a screen-stability model; the QUIET_SECONDS
// window maps to N = round(quiet / interval) identical snaps (0.6s -> 3).
const SNAPSHOT_INTERVAL = 0.2;
const QUIET_SECONDS = 0.6;

// Every idle wait is deadline-bounded (ADR gate 4: a never-converging spinner
// never settles, so the deadline is load-bearing). Python's byte-pump had no
// deadline because a real TUI always falls silent; the snapshot model adds this
// grace as the max active-churn we tolerate before proceeding without quiet.
...
export const TARGETS: Record<"claude" | "codex", Target> = {
  claude: {
    ...
    // CLAUDE_CONFIG_DIR from --agentwrap-profile <name>.
...
export function extractClaudeProfile(
  ...
  let i = 0;
  while (i < args.length) {
    if (args[i] === "--agentwrap-profile" && i + 1 < args.length) {
      profile = args[i + 1];
      i += 2;
    } else if (args[i].startsWith("--agentwrap-profile=")) {
...
/**
 * Capture the visible pane as newline-joined, per-row rstripped text — the
 * `"\n".join(rstrip rows)` shape the parsers expect. `-p` prints the visible
 * (alt-screen, when active) buffer; `-J` rejoins terminal-wrapped rows so a
 * sentinel split across a wrap still lands on one line. Never `-a` (it errors
 * when the pane is not on the alternate screen). A dead/gone pane returns "".
 */
 ...
 *  gone — the tmux equivalent of pexpect's EOF/EIO closed-output signal. */
...
/**
 * Type the slash command as three send-keys calls (ADR 0001): clear any
 * partially-typed input (`C-u`), type the slash literally (`-l`, so tmux never
 * interprets it as key names), then submit (`Enter` -> CR). A quiet pump after
 * each keystroke lets Ink render before the next.
 */
...
export async function scrape(
  ...
  // we happen to be running in (agentusage's own CLAUDE.md, planctl state, etc.).
  ...
  try {
    ...
    // harness's AGENTUSAGE_FAKE_CASE) would otherwise leak in. Mirrors Python's
    // os.environ.copy() + overrides. TERM is excluded (delivered via `-f`).
...
/**
 * Thin debug entry so the driver is developable against the fake TUI before the
 * task-6 CLI lands: `bun src/scrape.ts --target claude [--command X] [--rows N]
 ...
 * the CLI (task 6) owns argv parsing and the discriminated JSON contract.
 */
