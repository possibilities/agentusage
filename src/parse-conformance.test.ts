...
import { type CaseMeta, deriveContract } from "../tests/conformance-derive";

/**
 * In-process parse conformance over the frozen corpus screens.
 *
 * Feeds every case's committed `screen.txt` through the TS parsers via the same
 ...
 * runner). This is the custody artifact that keeps the corpus contract verified
 * after Python is gone; the live end-to-end driver check lives in
 * `scripts/conformance.ts` (`bun run conformance`).
 */

const CORPUS_DIR = join(import.meta.dir, "..", "tests", "fixtures", "corpus");
...
describe("parse conformance over frozen corpus screens", () => {
  ...
  // corpus has 14 committed cases as of this epic.
