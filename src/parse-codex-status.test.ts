...
import { CodexStatusParseError, parse } from "./parse-codex-status";

// Mirrors test_parse_codex_status.py's pinned NOW (2026-05-15 12:00, -04:00).
// Codex keeps `now`'s own fixed offset across resets, so a fixed-offset zone
// makes every reset carry -04:00. Expected values are hand-computed off the
// Python fixtures, not re-derived by the parser.
