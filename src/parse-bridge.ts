...
/**
 * Parse-only bridge for the pytest parity harness. Reads panel text on stdin,
 * takes `--target claude|codex` and an optional `--now <offset-iso>`, and prints
 * one line to stdout: the parsed usage JSON (exit 0) or, on any thrown parser
 * error, `{"error_type": "<name>", "message": "..."}` (exit 1). `error_type` is
 * the thrown error's name so it equals the Python exception class name
 * (ClaudeUsageParseError, ClaudeUsageEndpointRateLimited, NoActiveSubscription,
 * CodexStatusParseError) the parity module asserts against.
 *
 * `now` is built per target to match each Python parser's timezone semantics:
 * claude reprojects the resolved reset to the system zone (Python's
 * `.astimezone()`), so it needs a real IANA zone; codex keeps `now`'s own fixed
 * offset (Python resolves via `datetime.replace`, never reprojecting), so it
 * keeps the argv offset as a fixed-offset zone.
 */
