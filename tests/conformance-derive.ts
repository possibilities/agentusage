...
import { NoActiveSubscription } from "../src/parse-claude-usage";
import { detectSignedOut, TARGETS } from "../src/scrape";
import {
  classifyParseError,
 ...
 * cannot drift from the live CLI. The in-process parse-conformance test asserts
 * this equals the committed `expected.json`; `generate.ts` re-emits the goldens
 * from it. The live end-to-end contract (real tmux driver, auth probe, reaping)
 * is `scripts/conformance.ts`; this covers only what the rendered screen decides.
 */
